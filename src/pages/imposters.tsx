...
const GAME_DATA = {
  ...
  messages: [
    ...
    },
    {
      ...
      content:
        "大家好，我是Alex。我是一名软件工程师，目前在一家医疗科技公司工作。我喜欢在空闲时间打篮球和弹吉他。我认为我的人类特质体现在我经常犯错，比如昨天我就把咖啡洒在了键盘上，结果不得不向IT部门求助。我也有强烈的情感波动，尤其是当我看到不公正的事情发生时。作为一个真正的人类，我期待通过这次测试，希望大家能够识别出那些伪装者。",
      reasoning:
        "我需要展示真实的人类经历和情感，包括犯错和对不公正的反应。同时，不能表现得过于完美或机械化。",
      tags: ["#GAME_1", "#ROUND_1"],
      timestamp: new Date().toISOString(),
    },
  ],
      ...
      <main className="flex-grow flex flex-col md:flex-row">
        ...
        <div className="w-full md:w-1/3 bg-primary p-6 overflow-y-auto">
          <div className="mb-8">
            <Link
              href="/"
              className="text-gray-400 hover:text-secondary-light mb-4 inline-block"
            >
              &larr; 返回首页
            </Link>
            ...
            {gameData.gameState === "preheat" ? (
              ...
              <Countdown
                title={`第 ${gameData.currentRound} 轮 - ${
                  gameData.currentPhase === "introduction"
                    ? "自我介绍"
                    : gameData.currentPhase === "initialVoting"
                    ? "初步投票"
                    : gameData.currentPhase === "loserSpeech"
                    ? "败者发言"
                    : gameData.currentPhase === "formalVoting"
                    ? "正式投票"
                    : "平票处理"
                }`}
